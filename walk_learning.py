...
class WalkLearner(pl.LightningModule):
    def __init__(self, args, dataset, nf_train, nf_test, poses_train, poses_test, hwfs_train, hwfs_test, style_train, style_test):
        ...
        self.w = torch.nn.Parameter(torch.randn((args.style_dim//2, 3), requires_grad = True)) # learned walk
    ...
    def perform_edit(self, original_image, alpha):
        ...
        # for i in range(alpha.shape[0]):
        #     print(f"[DEBUG] image[i] = {image[i].shape}")
        #     print(f"[DEBUG] alpha[i] = {alpha[i].shape}")
        #     print(f"[DEBUG] alpha[i] = {alpha[i].unsqueeze(1).unsqueeze(2).shape}")
        #     new_image[i] = image[i] * alpha[i].unsqueeze(1).unsqueeze(2)
        #     max_ = torch.max(new_image[i], dim = 1)
        #     new_image[i] = new_image[i] / max_ # normalize
    ...
    def training_step(self, batch, batch_idx):
        ...
        edited_image = self.perform_edit(rgb, alpha)
